 ...
 *
 * Vendored into Polliniate3 from lensflare-threejs-vanilla-main/src/LensFlare.js
 * (CC0 license). Three surgical edits vs upstream:
 *   1. `import * as THREE from 'three'` removed — THREE is a global in this project.
 *   2. `import { easing } from 'maath'` replaced with an inline critically-damped lerp.
 *   3. Default `lensDirtTexture` URL swapped from a remote i.ibb.co link to the
 *      vendored `assets/fx/lensDirtTexture.png`.
 * All other code is verbatim so future upstream diffs apply cleanly.
 */

// Inline replacement for `maath`'s easing.damp — same critically-damped spring
// behavior. Based on Game Programming Gems 4 (Lengyel).
const easing = {
  damp(obj, key, target, smoothTime, dt) {
    const omega = 2 / Math.max(smoothTime, 1e-4);
    const x = omega * dt;
    const exp = 1 / (1 + x + 0.48 * x * x + 0.235 * x * x * x);
    obj[key] = target + (obj[key] - target) * exp;
  },
};

// Globalized for Skibidi Tower (loaded via <script>, not ES modules).
let LensFlareParams = {}
...
function LensFlareEffect(
  ...
  const lensFlareMaterial = new THREE.ShaderMaterial({
    uniforms: {
      ...
      lensDirtTexture: { value: new THREE.TextureLoader().load('assets/fx/lensDirtTexture.png') },
    },
    /*GLSL */
    fragmentShader: `
  ...
  lensFlareMaterial.onBeforeRender = function (renderer, scene, camera) {
    ...
    if (lensFlareMaterial.uniforms.followMouse.value === true) {
      ...
      // If the container is tagged to skip raycast-based occlusion, keep the
      // flare at full opacity as long as the sun is in front of the camera
      // (flarePosition.z < 1 guard above). Otherwise raycast as upstream does.
      if (lensFlareContainer.userData && lensFlareContainer.userData.skipRaycast) {
        internalOpacity = flarePosition.z < 1 ? oldOpacity : 0;
      } else {
        raycaster.setFromCamera(projectedPosition, camera)
        // Allow the caller to provide a curated occluder list (e.g. just
        // terrain + foliage) to avoid spurious occlusion from transparent
        // overlays like dust, particles, snow, or other VFX quads.
        const targets = (lensFlareContainer.userData && lensFlareContainer.userData.occluders) || scene.children;
        ...
        if (intersects[0].object.visible) {
          ...
          if (intersects[0].object.material.transmission) {
            ...
            if (intersects[0].object.material.transparent) {
              ...
              // Upstream bug fix: the README documents `userData = { lensflare: 'no-occlusion' }`
              // but upstream compared userData itself to the string. Honor both.
              const ud = intersects[0].object.userData;
              const skip = ud === 'no-occlusion' || (ud && ud.lensflare === 'no-occlusion');
              if (skip) {
